...
export type LongShortConstructorType = {
  keyId: string;
  secretKey: string;
  paper?: boolean;
  usePolygon?: boolean;
...
class LongShort {
  ...
  private stockList: StockListType;
  private long: [];
  private short: [];
  private qShort: number | null;
  private qLong: number | null;
  private adjustedQLong: number | null;
  private adjustedQShort: number | null;
  private blacklist: Set<string>;
  private longAmount: number;
  private shortAmount: number;
  private timeToClose: number | null;
  private bucketPct: number;


  constructor ({ keyId, secretKey, paper = true, bucketPct = 0.25 }: LongShortConstructorType) {
    ...
    const stocks = ['DOMO', 'TLRY', 'SQ', 'MRO', 'AAPL', 'GM', 'SNAP', 'SHOP', 'SPLK', 'BA', 'AMZN', 'SUI', 'SUN', 'TSLA', 'CGC', 'SPWR', 'NIO', 'CAT', 'MSFT', 'PANW', 'OKTA', 'TWTR', 'TM', 'RTN', 'ATVI', 'GS', 'BAC', 'MS', 'TWLO', 'QCOM']
  ...
  async run () {
    ...
    const spin = setInterval(async () => {
      ...
      try {
        ...
        const closingTime = new Date(String(clock.next_close).substring(0, String(clock.next_close).length - 6))
        const currTime = new Date(String(clock.timestamp).substring(0, String(clock.timestamp).length - 6));

        // TODO Seems weird Date into Number, need to check
        this.timeToClose = Math.abs(Number(closingTime) - Number(currTime));
      ...
      if (this.timeToClose && this.timeToClose < (MINUTE * INTERVAL)) {
